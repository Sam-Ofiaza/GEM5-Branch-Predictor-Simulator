...
rm -rf build/X86
wait 
scons build/X86/gem5.opt -j $2 &>> $CUR_DIR_FROM_GEM5/$OUTPUT_PATH/$OUTPUT_DIR_NAME/build.log
wait
cd $CUR_DIR_FROM_GEM5
wait
$GEM5_DIR/build/X86/gem5.opt -d $OUTPUT_PATH/$OUTPUT_DIR_NAME/hmmer $SIM_PATH -c $HMMER_BENCHMARK_PATH -o $HMMER_ARG_PATH $BENCHMARK_ARGS &>> $OUTPUT_PATH/$OUTPUT_DIR_NAME/hmmer.log
$GEM5_DIR/build/X86/gem5.opt -d $OUTPUT_PATH/$OUTPUT_DIR_NAME/sjeng $SIM_PATH -c $SJENG_BENCHMARK_PATH -o $SJENG_ARG_PATH $BENCHMARK_ARGS &>> $OUTPUT_PATH/$OUTPUT_DIR_NAME/sjeng.log
