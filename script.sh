...
mkdir -p $BUILD_PATH/$OUTPUT_DIR_NAME && cp -rf gem5 $BUILD_PATH/$OUTPUT_DIR_NAME && mkdir -p $OUTPUT_PATH/$OUTPUT_DIR_NAME/hmmer $OUTPUT_PATH/$OUTPUT_DIR_NAME/sjeng && cd $GEM5_DIR && rm -rf build/X86 && scons ../$BUILD_PATH/$OUTPUT_DIR_NAME/build/X86/gem5.opt -j4 &>> ../$OUTPUT_PATH/$OUTPUT_DIR_NAME/build.log && cd .. && $BUILD_DIR/$OUTPUT_DIR_NAME/gem5/build/X86/gem5.opt -d $OUTPUT_PATH/$OUTPUT_DIR_NAME/hmmer $BUILD_DIR/$OUTPUT_DIR_NAME/gem5/configs/example/se.py -c $HMMER_BENCHMARK_PATH -o $HMMER_ARG_PATH $BENCHMARK_ARGS &>> $OUTPUT_PATH/$OUTPUT_DIR_NAME/hmmer.log && $BUILD_DIR/$OUTPUT_DIR_NAME/gem5/build/X86/gem5.opt -d $OUTPUT_PATH/$OUTPUT_DIR_NAME/sjeng $GEM5_DIR/configs/example/se.py -c $SJENG_BENCHMARK_PATH -o $SJENG_ARG_PATH $BENCHMARK_ARGS &>> $OUTPUT_PATH/$OUTPUT_DIR_NAME/sjeng.log && rm -rf $BUILD_PATH/$OUTPUT_DIR_NAME
